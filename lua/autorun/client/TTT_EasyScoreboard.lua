...
function EZS.TitleHandleShift( sb )
...
function EZS.AddTitleLabel( sb )
	...
	sb:AddColumn( heading, function( ply, label )
		local title = ply:GetNWString("customtitle", " ")
		local titlecolor = string.ToColor(ply:GetNWString("customtitlecolor", "157 19 229 255")) or string.ToColor("157,19,229,255")
		...
		label:SetName( "EZSD" )
			...
			label.Think = function( s )
				...
				if not title then return end
					...
					s:SetTextColor( titlecolor )
					
					end
			

		if ov_name then return ov_name end
		return ply:GetNWString("customtitle", " ") 
		 
		
		
	end, EZS.ColumnWidth )
	
	EZS.TitleHandleShift( sb )
